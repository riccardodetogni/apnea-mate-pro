...
interface Props { recipientName?: string; requesterName?: string; eventTitle?: string; eventId?: string; eventDate?: string; eventLocation?: string }

const EventJoinRequestEmail = ({ recipientName, requesterName, eventTitle, eventId, eventDate, eventLocation }: Props) => (
  <Html lang="it" dir="ltr">
    <Head />
    <Preview>Nuova richiesta di partecipazione all'evento</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO} alt="Apnea Mate" width="180" style={logo} />
        <Heading style={h1}>Nuova richiesta per il tuo evento 🌊</Heading>
        <Text style={text}>Ciao {recipientName || "organizzatore"}!</Text>
        <Text style={text}><strong>{requesterName || "Un freediver"}</strong> vuole partecipare al tuo evento:</Text>
        ...
        <Button style={button} href={`${SITE_URL}/events/${eventId}`}>Gestisci richiesta</Button>
...
export const template = {
  component: EventJoinRequestEmail,
  subject: (d: any) => `Nuova richiesta per "${d.eventTitle || "il tuo evento"}"`,
  displayName: "Event join request",
  previewData: { recipientName: "Marco", requesterName: "Luca", eventTitle: "Stage Apnea Profonda", eventId: "abc", eventDate: "20-22 luglio 2026", eventLocation: "Dahab" },
