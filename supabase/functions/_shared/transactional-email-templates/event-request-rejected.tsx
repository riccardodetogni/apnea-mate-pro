...
interface Props { recipientName?: string; eventTitle?: string; eventDate?: string; eventLocation?: string }

const EventRequestRejectedEmail = ({ recipientName, eventTitle, eventDate, eventLocation }: Props) => (
  <Html lang="it" dir="ltr">
    <Head />
    <Preview>Partecipazione all'evento non approvata</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO} alt="Apnea Mate" width="180" style={logo} />
        <Heading style={h1}>Richiesta non approvata</Heading>
        <Text style={text}>Ciao {recipientName || "freediver"},</Text>
        <Text style={text}>Purtroppo la tua richiesta di partecipazione non è stata approvata:</Text>
        ...
        <Text style={text}>Esplora altri eventi disponibili nella community!</Text>
        <Button style={button} href={`${SITE_URL}/community`}>Esplora eventi</Button>
...
export const template = {
  component: EventRequestRejectedEmail,
  subject: (d: any) => `Partecipazione non approvata per "${d.eventTitle || "l'evento"}"`,
  displayName: "Event request rejected",
  previewData: { recipientName: "Luca", eventTitle: "Stage Apnea Profonda", eventDate: "20-22 luglio 2026", eventLocation: "Dahab" },
