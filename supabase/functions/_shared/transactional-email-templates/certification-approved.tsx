...
const CertificationApprovedEmail = ({ recipientName, roleLabel, isInstructor }: Props) => (
  <Html lang="it" dir="ltr">
    <Head />
    <Preview>Certificazione approvata!</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img src={LOGO} alt="Apnea Mate" width="180" style={logo} />
        <Heading style={h1}>Certificazione verificata! 🎉</Heading>
        <Text style={text}>Ciao {recipientName || "freediver"}!</Text>
        <Text style={text}>La tua richiesta di certificazione è stata <strong>approvata</strong>.</Text>
        <Container style={cardSuccess}>
          <Text style={cardTitle}>Nuovo ruolo: {roleLabel || "Freediver Certificato"}</Text>
          <Text style={cardMeta}>Ora hai accesso a funzionalità aggiuntive!</Text>
        </Container>
        <Text style={text}>Come {(roleLabel || "freediver certificato").toLowerCase()}, ora puoi:</Text>
        <Text style={text}>
          • Creare sessioni di apnea<br/>
          • Gestire partecipanti<br/>
          {isInstructor && <>• Creare gruppi e scuole<br/></>}
        </Text>
        <Button style={button} href={`${SITE_URL}/profile`}>Vai al profilo</Button>
...
export const template = {
  component: CertificationApprovedEmail,
  subject: "Certificazione approvata! 🎉",
  displayName: "Certification approved",
  previewData: { recipientName: "Marco", roleLabel: "Istruttore", isInstructor: true },
