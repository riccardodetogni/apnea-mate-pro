...
import { O2TableConfig as O2Config, formatTime } from "@/types/training";
...
interface O2TableConfigProps {
  onStart: (config: O2Config, customSteps?: { phase: "breathe" | "hold"; durationSeconds: number; round: number }[]) => void;
  onBack: () => void;
}
...
const computeRows = (config: O2Config): RowData[] => {
  const rows: RowData[] = [];
  for (let i = 0; i < config.rounds; i++) {
    const holdTime = config.startHoldSeconds + i * config.increaseStep;
    rows.push({ breathe: config.breathSeconds, hold: holdTime });
  }
  return rows;
};

export const O2TableConfig = ({ onStart, onBack }: O2TableConfigProps) => {
  const [config, setConfig] = useState<O2Config>({
    rounds: 8,
    startHoldSeconds: 60,
    breathSeconds: 120,
    increaseStep: 15,
  ...
  const { presets, savePreset, deletePreset, updatePreset } = useTrainingPresets("o2");

  const rows = customRows ?? computeRows(config);
  const totalSeconds = rows.reduce((acc, r) => acc + r.breathe + r.hold, 0);

  const updateConfig = (patch: Partial<O2Config>) => {
    setConfig(c => ({ ...c, ...patch }));
    setCustomRows(null);
    setEditingCell(null);
  ...
  const loadPreset = (preset: typeof presets[0]) => {
    const c = preset.config as O2Config;
    setConfig(c);
  ...
  const presetSummary = (preset: typeof presets[0]) => {
    const c = preset.config as O2Config;
    return `${c.rounds}r · ${formatTime(c.startHoldSeconds)}`;
  };
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <h2 className="text-xl font-bold text-foreground">{t("o2Table")}</h2>
                ...
                <div
                  ...
                  className={`!rounded-xl !p-3 !min-w-[140px] cursor-pointer flex-shrink-0 relative group ${
                    selectedPresetId === p.id
                      ? 'bg-gradient-to-br from-[hsl(30,90%,55%)] to-[hsl(15,85%,50%)] border border-white/30 rounded-xl'
                      : 'card-session'
                  }`}
      ...
      <div className="space-y-5">
        ...
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">
            {t("startHoldTime")}: {formatTime(config.startHoldSeconds)}
          </label>
          <Slider value={[config.startHoldSeconds]} onValueChange={([v]) => updateConfig({ startHoldSeconds: v })} min={15} max={180} step={15} />
        </div>
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">
            {t("breathTime")}: {formatTime(config.breathSeconds)}
          </label>
          <Slider value={[config.breathSeconds]} onValueChange={([v]) => updateConfig({ breathSeconds: v })} min={30} max={300} step={15} />
        </div>
        <div>
          <label className="text-sm text-muted-foreground mb-2 block">
            {t("increaseStep")}: {config.increaseStep}s
          </label>
          <Slider value={[config.increaseStep]} onValueChange={([v]) => updateConfig({ increaseStep: v })} min={5} max={30} step={5} />
        </div>
      </div>
